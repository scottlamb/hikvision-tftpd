...
class Server(object):
    ...
    def __init__(self, handshake_addr, tftp_addr, filename: str,
                 file_contents: bytes) -> None:
        assert(type(file_contents) == bytes)
    ...
    def _set_block_size(self, block_size: int):
        # TODO: Don't mutate overall server for a single transfer. Use some kind
        # of per-transfer state
        ...
        print('Serving %d-byte %s (block size %d, %d blocks)' % (
            len(self._file_contents), self._filename, self._block_size,
            self._total_blocks))
    ...
    def _parse_options(self, pkt: bytes):
    ...
    def _tftp_read(self):
        ...
        if pkt.startswith(self._tftp_rrq_prefix):
            ...
            if b'blksize' in options:
                self._set_block_size(int(options[b'blksize']))
    ...
    def _tftp_options_ack(self, addr):
        self._check_total_block_limit()
        pkt = (struct.pack('>H', self._TFTP_OPCODE_OACK) +
               b'blksize\x00%d\x00' % self._block_size)
        self._tftp_sock.sendto(pkt, addr)

    def _tftp_maybe_send(self, prev_block: int, addr):
        block = prev_block + 1
